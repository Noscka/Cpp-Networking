        ...
        try
        {
            ...
            while (!error)
            {
                ...
                case ServerRequest::Update:
                    ...
                    wprintf(std::wstring(L"Bytes sent: " + std::to_wstring((int)ServerNamespace::ServerFunctions::UploadFile(&socket, ServerNamespace::FilePathStorage::StaticPaths(ServerNamespace::FilePathStorage::StaticPaths::clientFile).GetFilePath(), 0, &InfoString, true)) + L"\n").c_str());
                    break;
